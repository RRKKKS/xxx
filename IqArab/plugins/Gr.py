...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**🝳 ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @SOURCE_NEON**")
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "🝳 ⦙   هذه قناة إختبار أُنشئت بإستعمال تليثون نيون"
    else:
        descript = "🝳 ⦙   هذه المجموعه إختبار أُنشئت بإستعمال تليثون نيون"
