...
LOGS = logging.getLogger("تليثون نيون \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://t.me/vd_d_dd/29", caption="🝳 ⦙ تـمّ  اعـادة تشـغيل\n تليثـون نيون ✓  :  [V1 ] .\n\n🝳 ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n🝳 ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n🝳 ⦙ القناة الرسمية تليثون نيون : @SOURCE_NEON\n🝳 ⦙ فارات سورس تليثون  :@SOURCE_NEON \n🝳 ⦙ كلايش تليثون :  @SOURCE_NEON\n 🝳 ⦙التحديثات والاضافات :  @SOURCE_NEON\n",                buttons=[(Button.url("مطور نيون الرسمي", "https://t.me/R_r_B0"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def iqchn():
    ...
    try:
        ...
        await iqthon(JoinChannelRequest("@SOURCE_NEON"))
    except BaseException:
        pass
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون نيون المدفوع", iqthon, Config.TG_BOT_USERNAME, descript  ,  iqphoto1 )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون نيون الخاص", iqthon, Config.TG_BOT_USERNAME, descript    , iqphoto2  )
